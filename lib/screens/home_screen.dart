...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Consumer<VpnState>(builder: (context, vpn, _) {
      ...
      final disconnecting = vpn.status == VpnStatus.disconnecting;
        ...
        body: SafeArea(
          child: Column(children: [
            ...
            Expanded(flex: 2, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              GestureDetector(
                onTap: vpn.busy ? null : vpn.toggle,
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(width: 4, color: connected ? AppTheme.success : connecting || disconnecting ? AppTheme.accent : AppTheme.textSecondary.withOpacity(0.3))),
                  child: Icon(
                    connected ? Icons.stop_rounded : connecting || disconnecting ? Icons.hourglass_top_rounded : Icons.play_arrow_rounded,
                    size: 72,
                    color: connected ? AppTheme.success : connecting || disconnecting ? AppTheme.accent : AppTheme.textSecondary))),
              const SizedBox(height: 20),
              AnimatedSwitcher(
                duration: const Duration(milliseconds: 300),
                child: Text(
                  connected ? 'ПОДКЛЮЧЕНО' : connecting ? 'ПОДКЛЮЧЕНИЕ...' : disconnecting ? 'ОТКЛЮЧЕНИЕ...' : 'НАЖМИТЕ ДЛЯ ПОДКЛЮЧЕНИЯ',
              ...
              if (vpn.error != null) ...[
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(color: AppTheme.danger.withOpacity(0.15), borderRadius: BorderRadius.circular(12), border: Border.all(color: AppTheme.danger.withOpacity(0.3))),
                    child: Row(children: [
                  ...
                  Expanded(child: Container(
                    ...
                    child: TextField(
                      ...
                      decoration: const InputDecoration(
                        hintText: 'URL подписки...',
                        hintStyle: TextStyle(color: AppTheme.textSecondary, fontSize: 13),
                        border: InputBorder.none,
                        contentPadding: EdgeInsets.symmetric(horizontal: 14, vertical: 12))))),
                  const SizedBox(width: 8),
                  FilledButton(
                    onPressed: () => vpn.addSubscriptionFromUrl(_urlCtrl.text),
                    style: FilledButton.styleFrom(minimumSize: const Size(72, 44), padding: EdgeInsets.zero, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                    child: const Text('Fetch', style: TextStyle(fontWeight: FontWeight.w700))),
                  const SizedBox(width: 6),
                  OutlinedButton(
                    onPressed: () => vpn.addSubscriptionFromClipboard(),
                    style: OutlinedButton.styleFrom(minimumSize: const Size(44, 44), padding: EdgeInsets.zero, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), side: BorderSide(color: AppTheme.card)),
                    child: const Icon(Icons.content_paste_rounded, size: 18)),
                ]),
                const SizedBox(height: 8),
                Row(children: [
                  ChoiceChip(
                    label: const Text('TCP', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600)),
                  ...
                  const SizedBox(width: 6),
                  ChoiceChip(
                    label: const Text('HTTP', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600)),
                  ...
                  const SizedBox(width: 8),
                  OutlinedButton(
                    onPressed: vpn.allNodes.isEmpty ? null : () => vpn.pingAll(),
                    style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), minimumSize: Size.zero, tapTargetSize: MaterialTapTargetSize.shrinkWrap, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                  ...
                  : ScrollConfiguration(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (_, si) {
                          ...
                          int offset = 0;
                          for (var k = 0; k < si; k++) {
                            offset += vpn.subscriptions[k].nodes.length;
                          }
                          return Container(
                            ...
                            child: Column(children: [
                              InkWell(
                                onTap: () => vpn.toggleSubscription(si),
                                borderRadius: BorderRadius.circular(14),
                                child: Padding(
                                  padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
                                  child: Row(children: [
                                    AnimatedRotation(
                                      turns: sub.expanded ? 0.25 : 0,
                                    ...
                                    const SizedBox(width: 8),
                                    GestureDetector(
                                      onTap: () => vpn.removeSubscription(si),
                                      child: Container(
                                        width: 28, height: 28,
                              ...
                              if (sub.expanded)
                                ...List.generate(sub.nodes.length, (ni) {
                                  final gi = offset + ni;
  ...
  void _showLogs(BuildContext context, VpnState vpn) {
    showModalBottomSheet(
      context: context, backgroundColor: AppTheme.bg, isScrollControlled: true,
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx2, sc) => Column(children: [
          ...
          Expanded(child: ListView.builder(
            controller: sc, physics: const BouncingScrollPhysics(),
  ...
  void _showSettings(BuildContext context, VpnState vpn) {
    showModalBottomSheet(
      context: context, backgroundColor: AppTheme.bg,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
      builder: (ctx) => Padding(
